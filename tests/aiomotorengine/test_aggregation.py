...
class TestAggregation(AsyncTestCase):
    ...
    def add_users(self):
        ...
        self.io_loop.run_until_complete(User.objects.bulk_insert(users))
    ...
    def add_cities(self):
        ...
        self.io_loop.run_until_complete(City.objects.bulk_insert(cities))

    @async_test
    @asyncio.coroutine
    def test_can_aggregate_number_of_documents(self):
        result = yield from User.objects.aggregate.group_by(
            User.email,
            Aggregation.avg(
    ...
    def test_can_aggregate_with_unwind(self):
        result = yield from User.objects.aggregate.unwind(User.list_items).group_by(
            User.email,
            User.list_items,
            Aggregation.avg(
                User.number_of_documents, alias="number_of_documents"
    ...
    def test_can_aggregate_with_sorting(self):
        result = yield from User.objects.aggregate.order_by(
            User.number_of_documents, DESCENDING
        ).fetch()
    ...
    def test_can_average_city_population(self):
        results = yield from City.objects.aggregate.group_by(
            City.state,
            City.city,
    ...
    def test_can_average_city_population_by_raw_query(self):
        results = yield from City.objects.aggregate.raw([
            {
                '$group': {
