...
class TestQueryProjection(AsyncTestCase):
    ...
    def create_test_objects(self):
        ...
        cat1 = yield from Category.objects.create(
    ...
    def test_can_project_with_only(self):
        users = yield from User.objects.filter(last_name="Else")\
    ...
    def test_can_project_with_onlies_chain(self):
        user = yield from User.objects\
            .only("first_name", "embedded.test")\
            .only('last_name').get(first_name="Someone")
    ...
    def test_can_project_with_exclude(self):
        user = yield from User.objects.exclude("first_name")\
        ...
        user = yield from User.objects.exclude(User.first_name)\
            .get(first_name="Someone")
    ...
    def test_can_project_with_excludes_chain(self):
        users = yield from User.objects.filter(last_name="Else").exclude('_id')\
    ...
    def test_can_combine_only_and_exclude(self):
        user = yield from User.objects.only('first_name').exclude('_id')\
            .get(last_name="Else")
        ...
        users = yield from User.objects.only("email", "numbers")\
            .exclude('numbers').order_by(User.index).find_all()

        expect(users[0].email).to_equal("heynemann@gmail.com")
        expect(users[0].numbers).to_equal([])

        users = yield from User.objects.exclude('numbers')\
            .only("email", "numbers").order_by(User.index).find_all()
    ...
    def test_can_project_embedded_fields(self):
        user = yield from User.objects.only('embedded.embedded2.test')\
            .get(last_name="Else")
        ...
        user = yield from User.objects.exclude('embedded.test')\
            .exclude('email').get(last_name="Else")
    ...
    def test_only_failed_with_wrong_field_name(self):
        ...
        with expect.error_to_happen(
            ...
            yield from User.objects.only('embedded.wrong')\
                .get(last_name="Else")
    ...
    def test_exlude_failed_with_wrong_field_name(self):
        ...
        with expect.error_to_happen(
            ...
            yield from User.objects.exclude('embedded.wrong')\
                .get(last_name="Else")
    ...
    def test_fields_failed_with_wrong_field_name(self):
        with expect.error_to_happen(
            ...
            yield from User.objects.fields(wrong=QueryFieldList.ONLY)\
                .get(last_name="Else")
        ...
        with expect.error_to_happen(
            ...
            yield from User.objects\
                .fields(embedded__wrong=QueryFieldList.EXCLUDE)\
                .get(last_name="Else")
        ...
        with expect.error_to_happen(
            ...
            yield from User.objects.fields(slice__embedded__wrong=10)\
                .get(last_name="Else")

    @async_test
    @asyncio.coroutine
    def test_can_slice_lists_in_projection(self):
        users = yield from User.objects.fields(slice__numbers=2)\
            .order_by(User.index).find_all()
        ...
        users = yield from User.objects.fields(slice__numbers=(1, 2))\
            .order_by(User.index).find_all()
    ...
    def test_can_combine_slice_with_only_and_exlude(self):
        users = yield from User.objects.fields(slice__numbers=(1, 1))\
            .only('email').order_by(User.index).find_all()
        ...
        users = yield from User.objects.fields(slice__numbers=(1, 1))\
            .only('_id').order_by(User.index).find_all()
    ...
    def test_can_slice_lists_in_projection_with_negative_skip(self):
        ...
        users = yield from User.objects.fields(slice__numbers=(-2, 1))\
            .order_by(User.index).find_all()
    ...
    def test_can_project_with_all_fields(self):
        user = yield from User.objects.only('last_name').exclude('_id')\
            .all_fields().get(last_name="Else")
    ...
    def test_can_project_list_of_embedded_documents(self):
        posts = yield from Post.objects.exclude('_id').only('comments.text')\
            .find_all()
    ...
    def test_can_project_list_of_references(self):
        users = yield from User.objects.only('first_name', 'posts.title')\
    ...
    def test_document_is_partly_loaded(self):
        only_users = yield from User.objects.only('first_name')\
            .order_by(User.index).find_all()
        ...
        exclude_users = yield from User.objects.exclude('first_name')\
            .order_by(User.index).find_all()
        ...
        slice_users = yield from User.objects.fields(slice__numbers=2)\
            .order_by(User.index).find_all()
    ...
    def test_document_is_not_partly_loaded(self):
        ...
        all_fields_users = yield from User.objects.only('first_name')\
            .exclude('_id').all_fields().order_by(User.index).find_all()
    ...
    def test_document_failed_to_save_partly_loaded_document(self):
        user = yield from User.objects.only('first_name')\
            .get(first_name='Someone')
    ...
    def test_queryset_failed_to_save_partly_loaded_document(self):
        users = yield from User.objects.only('first_name')\
            .filter(first_name='Someone')\
            .order_by(User.index).find_all()
